...
CultivateImmortalityService.prototype = {
    ...
    SuperUserAddData: function (type, data) {
        ...
        if (type == 1) {
            ...
            if (inputs.ItemID == -1) {
                ...
                var equipmentItem =new EquipmentItem(inputs);
                ...
                equipmentItem =new EquipmentItem(inputs);
        ...
        //2 地图信息
            ...
            if (inputs.MapID == -1) {
                ...
                var mapItem =new MapItem(inputs);
                mapItem.MapID = mapID;
        ...
        // 3 怪物数据
            ...
            if (inputs.MonsterID == -1) {
                ...
                var monsterItem =new MonsterItem(inputs);
                ...
                monsterItem =new MonsterItem(inputs);
    ...
    _ExecBattleMonster(useraddress, monsterId) {
        ...
        var userItem = this.UserItem.get(useraddress);
        for (var i = 0; i < this.BattleMaxRound; i++) {
            userItem.HP -= monsterItem.MonsterAttach - userItem.Defense;
            monsterItem.MonsterHP -= userItem.Attach - monsterItem.MonsterDefense;
            if (userItem.HP <= 0) {
                return 0;
            }
            if (monsterItem.HP <= 0) {
    ...
    _rewardExp(useraddress, mapItem) {
        ...
        var levelneedexp = this.LevelExps.get(userItem.Level);
        if (levelneedexp) {
    ...
    _ExecOneBattle(useraddress, blocknum, mapID) {
        ...
        if (monsterRate < this.BossBattleRate) {
            ...
            var battleResult = _ExecBattleMonster(useraddress, monsterId);
            result.set("monster", this.MonsterRepo.get(monsterId));
            result.set("battleResult", battleResult);
            if (battleResult == 1) {
                ...
                if (rewardRate < mapItem.rewardRate) {
                    ...
                    var reward = _rewardItem(useraddress, itemId);
                    result.set("reward", reward);
                }
    ...
    CalBattleResult: function (newMapID) {
        ...
        for (var i = battleRecordItem.RecordStartBlockNumber; i < currentBlockHeight; i++) {
            var oneResult = _ExecOneBattle(userAddress, i, battleRecordItem.MapId);
            BattleResults.push(oneResult);
        }
        return BattleResults;
    },

    QueryBattleResult: function (newMapID,blockheight) {
        var userAddress = Blockchain.transaction.from;
        var battleRecordItem = this.BattleRecordRepo.get(userAddress);
        if (!battleRecordItem) {
            throw new Error("Useraddress: " + userAddress + " not registered!")
    ...
    QueryMonsters: function (monsterIndex) {
        var result = [];
        if (monsterIndex == -1) {
            for (var i = 0; i < this.MonsterSize; i++) {
    ...
    QueryEquipments: function (Index) {
        var result = [];
        if (Index == -1) {
            for (var i = 0; i < this.EquipmentSize; i++) {
