...
EquiSplit.prototype = {
  ...
  payBills: function(name) {
    ...
    var value = Blockchain.transaction.value;
    var amount = new BigNumber(value);
    var length = this.partnersmap.get(name).length;

    var amountToSend = amount.dividedToIntegerBy(length);
    this.partnersmap.get(name).forEach(function (item) {
      var result = Blockchain.transfer(item, amountToSend);
        if (!result) {
          throw new Error("transfer failed. " + item + " amount " + amountToSend);
        }
    }, this);
