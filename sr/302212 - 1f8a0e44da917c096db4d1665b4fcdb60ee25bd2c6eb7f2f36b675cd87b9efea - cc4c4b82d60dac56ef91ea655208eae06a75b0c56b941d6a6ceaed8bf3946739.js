...
TwentyOneDaysContract.prototype = {
    ...
    createActivity: function (title, description, startDate, endDate) {
        if(Blockchain.transaction.value.lt(new BigNumber(100000000000000))){
            throw new Error("Deposit should be greater than 0.0001NAS.");
        }
    ...
    _updateActivityState: function(activity){
        ...
        if(activityFinished){            
            ...
            if(validCheckinDays === activityPeriod){
                ...
                var userBalance = new BigNumber(activity.balance.times(validCheckinDays).div(activityPeriod).toFixed(0));
                if(userBalance.lt(new BigNumber(0))){
                    userBalance = 0;
                }
                if(userBalance.gt(activity.balance)){
    ...
    withdrawMyDeposit: function(activityIndex){
        if(Blockchain.transaction.value.gt(new BigNumber(0))){
            throw new Error("Don't send any NAS when withdraw balance.");
        }
        ...
        currentActivity.balance = new BigNumber(0);
    ...
    updateAllActivityState: function(){
        if(Blockchain.transaction.value.gt(new BigNumber(0))){
            throw new Error("Don't send any NAS when update activity.");
        }
    ...
    checkDeveloperBonus: function(){
        if(Blockchain.transaction.value.gt(new BigNumber(0))){
            throw new Error("Don't send any NAS when check balance.");
        }
    ...
    getAllActivities: function(){
        if(Blockchain.transaction.value.gt(new BigNumber(0))){
            throw new Error("Don't send any NAS when get activities.");
        }
