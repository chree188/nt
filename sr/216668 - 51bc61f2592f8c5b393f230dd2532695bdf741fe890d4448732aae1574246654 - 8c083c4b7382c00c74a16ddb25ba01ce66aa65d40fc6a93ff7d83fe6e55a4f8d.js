...
BidToWinContract.prototype = {
    ...
    bid: function (phoneNo, nickName) {
        ...
        if (this.bidNumber >= 10) {

            var rnd = this.random();
            var winner = this.bidPool.get(rnd);

            var result = Blockchain.transfer(winner.address, 9 * 100000000000000000);
            if (!result) {
            ...
            result = Blockchain.transfer("n1GM4315J95rjZwB9gGfzZC47HRF6a3bfF2", 1 * 100000000000000000);
            if (!result) {
    ...
    random : function () {
        ...
        return parseInt(sum.toString().slice(-1));
    },
