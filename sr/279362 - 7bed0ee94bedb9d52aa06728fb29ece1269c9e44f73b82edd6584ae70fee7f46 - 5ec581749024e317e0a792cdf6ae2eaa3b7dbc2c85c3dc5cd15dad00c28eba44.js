...
ListRestaurant: function(){
	var fromaddr = Blockchain.transaction.from;
		...
		var Lsize = LocalContractStorage.get("NumberSize");
		var i=parseInt(Lsize);
		var qsize = LocalContractStorage.get("QueueSize");
		var j=parseInt(qsize);
		var TempNumber = new NumberInformation();
		...
		while(i>0){
			...
			TempNumber =this.NumberStorage.get(i.toString());
			while(j>0){
				TempQueue = this.QueueStorage.get(j.toString());
				if(TempQueue.userAddr == fromaddr && TempQueue.numId==TempNumber.numId){
					ismyque=1;
				}
				j--;
			}
					Restauinfo = this.RestaurantStorage.get(TempNumber.numFromAddr);
					AllInfo1={"numinfo":TempNumber,"restainfo":Restauinfo,"ismyque":ismyque};
					AllInfo.push(AllInfo1);
		...
		if(TempNumber.numTotal>0){
			...
			while(i>0){
				...
				if(istake=="n"){
					...
					UpdateNumber.numTotal = TempNumber.numTotal-1;
