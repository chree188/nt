...
GZB.prototype = {
    ...
    save: function (payer,receiveer,money,endtime){
     ...
     searchAuthor:function(author,p){
        ...
        var end   = (arr.length  > page * this.pageNum)? page * this.pageNum : arr.length;
        var count =   end - star;//count为计算该页有多少条记录
        var list = [];//存储该用户所有num的最新记录
        for (var i = count-1; i >=0; i--) {
            var num = arr[star+i]; //获取Num
            var record = this.getNumNewRecord(num);//取num账单下的最新记录
            return record;
            list.push(record);//存入num账单下的最新记录
        }
        return list;
     },
     //账户总页码
