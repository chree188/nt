...
StandardToken.prototype.recalculatePrices = function () {
  ...
  let realSupplyCoverage = totalSupplyNAS.eq(0) ? new BigNumber(1) : contractBalance.div(totalSupplyNAS); // a number from 0...Inf
  
  let supplyCoverage = 
    totalSupplyNAS.gt(contractBalance) ? new BigNumber(1) :
...
StandardToken.prototype.dispose = function (userSellPrice, userSellGoldRAW) {
  ...
  if (currentNASBalance.gt(requiredNASTokens)){
    throw new Error('We do not have enough NAS to proceed with the payment. Please try again later.');
  }
  ...
  if (transferResultFailed){
    throw new Error('Unable to transfer amount ' + requiredNASTokens + ' to ' + sellerAddress);
  }
