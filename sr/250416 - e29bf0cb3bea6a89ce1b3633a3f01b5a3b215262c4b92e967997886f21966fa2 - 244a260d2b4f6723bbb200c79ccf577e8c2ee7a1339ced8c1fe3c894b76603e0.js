...
StandardToken.prototype.recalculatePrices = function () {
  ...
  let totalSupplyNAS = totalSupply.div(numberTen.pow(decimals)).mul(goldPriceNAS);
  
  // Now we need to know, if we have enough funds, to cover all issued tokens at current price...
  let supplyCoverage = totalSupplyNAS.gt(0) ? contractBalance.div(totalSupplyNAS) : 1; // A number from 0 to ... If it is above 1, it is ok.
  ...
  let buyPrice = goldPriceNAS.mul(depositFee.add(holidayFee).add(1));
  ...
  let sellPrice = goldPriceNAS.mul(withdrawalFee.add(holidayFee).negated().add(1)).mul(supplyCoverage.lt(1) ? supplyCoverage : 1);
...
StandardToken.prototype.prices = function () {
  return {
    ...
    "buyPriceNAS": this.buyPrice.toPrecision(5), // At what price user can buy gold tokens
    "sellPriceNAS": this.sellPrice.toPrecision(5), // At what price user can sell gold tokens
