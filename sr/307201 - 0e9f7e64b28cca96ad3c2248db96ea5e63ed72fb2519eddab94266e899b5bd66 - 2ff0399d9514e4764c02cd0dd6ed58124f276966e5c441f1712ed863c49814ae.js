...
var MaimaiContractList = function () {
   ...
   LocalContractStorage.defineMapProperty(this, "mmDianzanMap");

   LocalContractStorage.defineMapProperty(this, "mmDianzanSize");
...
MaimaiContractList.prototype = {
   ...
   fetchMaimai: function (pageNumber) {
       ...
       for (var i = startIndex; i >= endIndex; i--) {
           ...
           maimaiString += data + "nebulas";
       }
       return maimaiString.substr(0, maimaiString.length - 7);
   },
    ...
    * @returns {string}
    */
   getDianzanList: function () {
       var user = Blockchain.transaction.from;
       var dianzanSize = 0;
       dianzanSize = this.mmDianzanSize.get(user);
       if (!dianzanSize) {
           return "";
       }
       var dianzanKeyString = "";
       for (var i = 0; i < dianzanSize; i++) {
           var dianzanMaimaiKey = this.mmDianzanMap.get(this._getDianzanKey(user, i));
           if (!dianzanMaimaiKey) {
               continue;
           }
           dianzanKeyString += dianzanMaimaiKey + "nebulas";
       }
       return dianzanKeyString.substr(0, dianzanKeyString.length - 7);
   },
   ...
   saveCommentOfMaimai: function (keyOfMaimai, commentValue, nativeIndex) {
       ...
       this.addFavorite(keyOfMaimai, "0");
       return index;
   },
    ...
    * @param isDianzan 是否是点赞
    */
   addFavorite: function (key, isDianzan) {
       this._getOneMaimaiContentByKey(key);
       var user = Blockchain.transaction.from;
       this._addConcern(key, user);
       if (isDianzan === "1") {
           this._addDianzan(key, user);
       }
   },

   //加关注
   _addConcern: function (key, user) {
       var tmpKey;
       var size = 0;
   ...
   //加点赞
   _addDianzan: function (key, user) {
       var tmpKey;
       var size = 0;
       size = this.mmDianzanSize.get(user);
       if (!size) {
           this.mmDianzanMap.set(this._getDianzanKey(user, 0), key);
           size = 1;
           this.mmDianzanSize.set(user, size);
           return "first insert: size = " + this.mmDianzanSize.get(user) + "; value = " + this.mmDianzanMap.get(this._getFavoriteKey(user, 0));
       } else {
           for (var i = 0; i < size; i++) {
               tmpKey = this.mmDianzanMap.get(this._getFavoriteKey(user, i));
               if (tmpKey === key) {
                   return;
               }
           }
           this.mmDianzanMap.set(this._getDianzanKey(user, size), key);
           size++;
           this.mmDianzanSize.set(user, size);
           return "insert: size = " + this.mmDianzanSize.get(user) + "; value = " + this.mmDianzanMap.get(this._getFavoriteKey(user, i));
       }
   },
   ...
   delFavoriteWithoutIndex: function (key) {
       ...
       //删除关注列表里面的数据
       this._delConcern(key, user);
       //删除点赞里面的数据
       this._delDianzan(key, user);
   },

   _delConcern: function (key, user) {
       var size = this.mmFavoriteSizeMap.get(user);
       var tmpKey;
   ...
   _delDianzan: function (key, user) {
       var size = this.mmDianzanSize.get(user);
       var tmpKey;

       for (var i = 0; i < size; i++) {
           tmpKey = this.mmDianzanMap.get(this._getDianzanKey(user, i));
           if (tmpKey === key) {
               this._doDelDianzan(i, size, user);
               break;
           }
       }
   },

   _doDelDianzan: function (index, size, user) {
       //将删除的点赞index之后的item都向前挪一位（避免分页出错）
       for (var i = index + 1; i < size; i++) {
           var key = this.mmDianzanMap.get(this._getDianzanKey(user, i));
           this.mmDianzanMap.set(this._getDianzanKey(user, i - 1), key);
       }
       //挪完之后删除当前用户的最后一个关注item
       this.mmDianzanMap.del(this._getDianzanKey(user, size - 1));
       //将当前用户的关注数量减一
       this.mmDianzanSize.set(user, size - 1);
   },
   ...
   _getDianzanKey: function (user, index) {
       return user + "_dianzan_" + index;
   },
