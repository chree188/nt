...
StandardToken.prototype.internal = function () {
  ...
  return {
    ...
    "contractBalanceNAS": this._contractBalance.div(this._decimalsValueNAS), // How many NAS we have on balance.
...
StandardToken.prototype.purchase = function (userBuyPrice) {
  ...
  if (tokensToIssue.lt(1)){
    throw new Error('Unable to issue tokens, please send more NAS.');
  }
...
StandardToken.prototype.withdraw = function (rawWEIamount) {
  ...
  if (currentNASBalance.sub(nasAmountTokens).div(totalSupplyNASTokens).lt(1.25)){
    throw new Error("After withdrawal supply coverage will become less than 125%, you can withdraw " + currentNASBalance.sub(totalSupplyNASTokens.mul(1.25)).dividedToIntegerBy(1) + " WEI");
  }
  ...
  if (
    ...
    throw new Error("You can withdraw only earnings " + currentTotalEarnings.dividedToIntegerBy(1) + " WEI. You will be able to withdraw more after supply coverage will become more than 200%. You can withdraw now " +currentNASBalance.sub(totalSupplyNASTokens.mul(2)).dividedToIntegerBy(1) + " WEI");
  }
