...
LotteryContract.prototype = {
    ...
    bid: function(amount,target) {
        ...
        if (target == true) { // true for BIG, false for SMALL
            ...
            this.balance = new BigNumber(this.bigbidsbalance).plus(new BigNumber(value));
            this.bigbidsno++;
        } else {
            this.addrbigbids.set(this.smallbidsno,from);
            this.smallbids.set(from, value);
            this.balance = new BigNumber(this.smallbidsbalance).plus(new BigNumber(value));
            this.smallbidsno++;
        }
        return "Success";
    },
    ...
    bidend: function() {
       ...
       if ( height - this.startheight == this.maxblocks || (this.bigbidsno >0 && this.smallbidsno >0 && this.bigbidsno+this.smallbidsno>=this.maxplayers) ) {
            this.started = 0;
            if ( this.endheight == 0 ) {
           ...
           var state = "BBN: "+ this.bigbidsno + " | SBN: " + this.smallbidsno;
           if ( this.bigbidsno <= 0) {
               return "No one bids for BIG" + state;
           } else if ( this.smallbidsno <= 0) {
               return "No one bids for SMALL" + state;
           }
           var totalplayers = this.bigbidsno+this.smallbidsno;
           state +=  + " | TP: "+ totalplayers;
           if ( totalplayers < this.maxplayers ) {
               return "Still need more players. CURRENT "+ totalplayers.toString();
           }

           return "unhandled exception found" + state;
       }
       console.log("SH: "+ (height - this.startheight) + " | BBN: "+ this.bigbidsno + " | SBN: " + this.smallbidsno);
