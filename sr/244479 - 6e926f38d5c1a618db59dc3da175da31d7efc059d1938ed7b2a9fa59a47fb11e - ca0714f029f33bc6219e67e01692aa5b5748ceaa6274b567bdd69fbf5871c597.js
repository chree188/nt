...
var StandardToken = function () {
  ...
  LocalContractStorage.defineProperties(this, {
    ...
    "_tags": {
      ...
      stringify: function (arrayOfStrings) {
        if (typeof (arrayOfStrings) !== "array"){
  ...
  LocalContractStorage.defineProperties(this, {
    ...
    '_depositFee': { // Fee for buying GOLD (depositing NAS). It is in range from 0 to 1000000, where 1000000 is 100% fee.
      parse: function (storedIntegerString) {
        return storedIntegerString / 1000000;
      },
      stringify: function (receivedInteger) {
        if (!Number.isInteger(receivedInteger)){
          throw new Error("Fee must be specified as Integer number");
        }
        else if (receivedInteger < 0 || receivedInteger > 1000000){
...
StandardToken.prototype = {
  ...
  _notify: function (eventType, eventData){
    ...
    return Event.Trigger(this.name(), eventObject);
  },
  ...
  balanceOf: function (walletAddress) {
    ...
    var balance = this.balances.get(walletAddress) || new BigNumber(0);
  ...
  transferFrom: function (senderAddress, receiverAddress, tokensAmount) {
    ...
    if (
      !value.lt(0) // Transfer is non-negative
      && senderBalance.gte(value) // Sender has enough funds
      && allowedBalance.gte(value) // Spender has enough allowance.
    ) {
  ...
  approve: function (spenderAddress, previousValue, newValue) {
    ...
    if (value.lt(0) || senderBalance.lt(value)) {
  ...
  // My Custom methods. Should be removed from NRC-20 standart
  ...
  config: function (settingsObject){
    ...
    for (var setting in settingObject){
      ...
      var newValue = settingObject[setting];
