...
NebupollsContract.prototype =
{
  ...
  vote: function(id, vote)
  {
    ...
    else
    {
      ...
      const oldIndex = topPolls.indexOf(id)
      let index = oldIndex
      ...
      if(index >= 0)
      {
        ...
        while(index > 0)
        {
          ...
          const totalVotes = this.polls.get(topPolls[index]).totalVotes
        ...
        topPolls.splice(oldIndex, 1)
        topPolls.splice(newIndex, 0, id)

        ee += "4_(" + topPolls.length + ")"

      }
      else
      {
        let insertLocation = topPolls.length

        ee += "5_(" + topPolls.length + ")"

        while(insertLocation > 0)
        {
          if(this.polls.get(topPolls[insertLocation - 1]).totalVotes >= poll.totalVotes)
          {
            break
          }
          else
        ...
        {
          topPolls.splice(insertLocation, 0, id)

          ee += "id is " + id

          if(topPolls.length > TOP_POLLS_COUNT)
          {
            ee += "6.5_(" + topPolls.length + ")"

            topPolls = topPolls.slice(0, TOP_POLLS_COUNT)
          }

          ee += "7_(" + topPolls.length + ")"
