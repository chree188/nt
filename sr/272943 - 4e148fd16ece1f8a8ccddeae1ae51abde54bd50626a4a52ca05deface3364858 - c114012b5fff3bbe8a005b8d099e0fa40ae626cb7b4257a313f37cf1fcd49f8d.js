...
ReleaseMyInfo: function (PlateNo,LeaveMessage){
	...
	LeaveMessage = LeaveMessage.trim();
		...
		if (PlateNo.length>120)
		{
			throw new Error("PlateNo too long");
		}
		if (LeaveMessage.length>180)
		{
			throw new Error("LeaveMessage too long");
		}
...
GetCallMeInfo: function (){
		...
		if(Lsize>0){
			...
			while(i>0){
				TempLicense=this.LicensePN.get(i.toString());
				if(TempLicense.CarOwnerAddr==fromaddr && TempLicense.ReleaseStatus==1){
					//
					TempPlateNo=TempLicense.PlateNo;
					j=Csize;
					while(j>0){
