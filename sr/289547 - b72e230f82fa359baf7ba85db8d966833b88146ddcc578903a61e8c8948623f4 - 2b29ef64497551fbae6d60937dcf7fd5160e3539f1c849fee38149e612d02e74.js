...
TTBox.prototype = {
    ...
    _getBoxKey:function(id){
        ...
        var diffmin=Math.floor((now-box.birth)/(20));
        if(diffmin>=10){
            diffmin+="";
            diffmin=parseInt(diffmin.substr(diffmin.length-2,2));
        }
    ...
    openBox:function(id,key){
        ...
        var ids=this.bidToRecordIds.get(id);
        if(!ids) ids=[];
        ids.push(this.recordIndex);
        this.bidToRecordIds.set(id,ids);
        this.indexToRecord.set(this.recordIndex,record);
    ...
    getBoxRecord:function(bid){
        ...
        var arr=[];
        rids.forEach(r => {
            var rec=this.indexToRecord.get(r)
            arr.push(rec);
        });
        return arr;
    },
