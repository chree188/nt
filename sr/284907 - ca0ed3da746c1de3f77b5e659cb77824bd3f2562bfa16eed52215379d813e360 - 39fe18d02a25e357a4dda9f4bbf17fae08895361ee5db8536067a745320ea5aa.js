...
Blog.prototype = {
    ...
    show_user_posts: function (limit, offset) {
        ...
        for (var i = 0; i < this.post_capacity; i++) {
            ...
            var author = this.post_author_indexies.get(i);
            if (author && author == user_address) {
    ...
    post: function (title, content) {
        ...
        var index = this.post_capacity;
        post.post_id = index;
        this.post_author_indexies.put(index, post.address);
    ...
    edit_post: function (post_id, title, content) {
        ...
        for (var i = 0; i < this.post_capacity; i++) {
            ...
            if (post && post_id == post.post_id) {
                if (post.author != Blockchain.transaction.from) {
    ...
    export_data: function () {
        ...
        if (authed_address.find(function(address) { return address == user_address}) == undefined) {
