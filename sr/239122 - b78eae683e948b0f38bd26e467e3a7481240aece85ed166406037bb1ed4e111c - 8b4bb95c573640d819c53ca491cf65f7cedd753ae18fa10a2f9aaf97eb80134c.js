...
VoteContract.prototype = {
    ...
    Voting: function (id, choice) {
        ...
        // console.log("合约查到的地址", voter_address);
        // if (Blockchain.block.timestamp >= this.votes.get(id).endTime) {
        //     throw new Error("vote is end");
        // } else {
        //     for(var voter in this.votes.get(id).voters){
        //         if (voter_address == voter){
        //              throw new Error("had voted");
        //         }else {
        //         if (choice == "choice_a") {
        //             this.votes.get(id).choice_a_tickets += 1;
        //             this.votes.get(id).voters.push(voter_address)
        //         } else if (choice == "choice_b") {
        //             this.votes.get(id).choice_b_tickets += 1;
        //             this.votes.get(id).voters.push(voter_address)
        //         } else {
