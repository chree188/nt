...
MBFlappyBear.prototype = {
    ...
    _updateTopScore: function(owner, scoreInt) {
        ...
        for (var j = 0; j < this.maxTopScoreSize; j++) {
            var currentScoreInt = parseInt(bestScoreList[j]);
            if (scoreInt > currentScoreInt) {
                bestScoreList.splice(j, 0, scoreInt.toString());
                bestScoreOwnerList.splice(j, 0, owner);

                if (bestScoreList.length > this.maxTopScoreSize) {
                    bestScoreList.pop();
