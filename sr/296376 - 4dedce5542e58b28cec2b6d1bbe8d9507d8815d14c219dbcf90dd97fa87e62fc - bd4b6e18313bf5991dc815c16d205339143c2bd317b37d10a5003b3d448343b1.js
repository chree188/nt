...
//        if (value.toNumber() <= 0.000001) {
//            throw new Error("需花费至少0.000001nas！");
//        }
    ...
    search: function(type, key) {
        ...
        if (type === "address") {
//            if (key !== from) {
//                throw new Error("你没有查询改用户的权限，仅限查询自己的账号");
//            }
        ...
        if (type === "code") {
            var eventid = from + "_" + key;
            var isHasCode = this.event.get(eventid);
            if (isHasCode) {
    ...
    list: function() {
        ...
        if (from !== "n1GVzYoxGgWG8i2VungBQcZ9CZcbzaztmYD") {
            throw new Error("抱歉，你暂时没有权限查看全部数据");
        }
        var result = [];
