  ...
  LocalContractStorage.defineProperties(this, {
    ...
    "_depositFee": { // Fee for buying GOLD (depositing NAS). It should be a share, from 0 to 1, rounded to 5 decimals
      ...
      stringify: function (receivedNumber) {
        ...
        else if (number.lt(0) || number.isGreaterThan(1)){
    ...
    "goldPrice": { // Gold price in USD per 1 gramm, we can just store it
      ...
      stringify: function (receivedNumber) {
        ...
        else if (number.lt(0) || number.isGreaterThan(1000000)){
...
StandardToken.prototype = {
  ...
  init: function (name, symbol, decimals, totalSupply, description, help, tags, userInterface) {
    ...
    // Otherwise, we store total supply with the necessary precision.
      ...
      this._totalSupply = checkTotalSupply.multipliedBy(new BigNumber(10).exponentiatedBy(decimals));
    }
  ...
  transfer: function (receiverAddress, tokensAmount) {
    ...
    this.balances.set(senderAddress, senderBalance.minus(value));
    ...
    this.balances.set(receiverAddress, receiverBalance.plus(value));
  ...
  transferFrom: function (senderAddress, receiverAddress, tokensAmount) {
    ...
    if (value.lt(0)){ // Transfer must be non-negative
      ...
      this.balances.set(senderAddress, senderBalance.minus(value));
      this.allowed.set(allowanceString, allowedBalance.minus(value));
...
StandardToken.prototype.recalculatePrices = function () {
  ...
  let goldPriceNAS = this.goldPrice.dividedBy(this.nasPrice);
  ...
  let totalSupplyNAS = this._totalSupply.dividedBy(numberTen.exponentiatedBy(this._decimals)).multipliedBy(goldPriceNAS);
  
  // Now we need to know, if we have enough funds, to cover all issued tokens at current price...
  let supplyCoverage = this._contractBalance.dividedBy(totalSupplyNAS); // A number from 0 to ... If it is above 1, it is ok.
  // totalSupplyNAS.isZero() ? this._contractBalance.dividedBy(totalSupplyNAS) : 1;
  ...
  let buyPrice = goldPriceNAS.multipliedBy(this._depositFee.plus(this._isHoliday ? this._holidayFee : 0).plus(1));
  ...
  let sellPrice = goldPriceNAS.multipliedBy(this._withdrawalFee.plus(this._isHoliday ? this._holidayFee : 0).negated().plus(1)).multipliedBy(supplyCoverage.lt(1) ? supplyCoverage : 1);
