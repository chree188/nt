...
monContract.prototype = {
    ...
    getMonNums: function () {
        ...
        if (userMonNums !== 0) {
            var lastHash = this.userMonIndex.get(userAddress + "." + i)
            var lastMon = this.userMonInfo.get(lastHash)
            var lastCatchTime = lastMon.catchTime
            if (lastCatchTime - Blockchain.transaction.timestamp < 15) {
                throw new Error("Interval less than 15 seconds");
            }
        }
    ...
    userMonsList: function (limit, offset) {
        var userAddress = Blockchain.transaction.from
        var userMonNums = this.userMonNums.get(userAddress) * 1
        if (userMonNums == 0) {
            return []
        ...
        for (var i = offset; i < number; i++) {
            // result.push(i)
            // var mon = this.monInfo.get(i)
            var txHash = this.userMonIndex.get(userAddress + "." + i)
            var mon = this.userMonInfo.get(txHash)
            if (mon !== null) {
                result.push(mon)
            }
        }
    ...
    t1: function () {
        var userAddress = Blockchain.transaction.from
        return [this.userMonNums.get(userAddress) * 1, this.userMonNums.get(userAddress)]
    },
    t2: function () {
        var userAddress = Blockchain.transaction.from
        var userMonNums = this.userMonNums.get(userAddress) * 1
