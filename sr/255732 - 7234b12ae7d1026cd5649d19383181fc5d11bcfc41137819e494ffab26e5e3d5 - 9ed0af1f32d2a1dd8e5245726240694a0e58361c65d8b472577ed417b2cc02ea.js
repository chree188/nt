...
StandardToken.prototype.recalculatePrices = function () {
  ...
  let totalSupplyNAS = totalSupply.div(new BigNumber(10).pow(decimals)).mul(goldPriceNAS);
  
  // Now we need to know, if we have enough funds, to cover all issued tokens at current price...
  let supplyCoverage = totalSupplyNAS.gt(0) ? contractBalance.div(totalSupplyNAS) : new BigNumber(1); // A number from 0 to ... If it is above 1, it is ok.
  ...
  let unsecuredShare = supplyCoverage.lt(1) ? new BigNumber(1).sub(supplyCoverage) : new BigNumber(0);
  
  let sellPriceUnsecured = goldPriceNAS.mul(unsecuredShare);
  let sellPriceFee = goldPriceNAS.mul(withdrawalFee.add(holidayFee)).mul(unsecuredShare);
  ...
  if (this.nasPrice && this.goldPrice){
    let buyPriceWithPrecision = buyPrice.toPrecision(5);
    let sellPriceWithPrecision = sellPrice.toPrecision(5);
...
StandardToken.prototype.prices = function () {
  return {
    ...
    "contractBalanceNAS": this._contractBalance, // How many NAS we have on balance.
...
StandardToken.prototype.purchase = function (userBuyPrice) {
  ...
  let validDifference = this._priceValidDifference.negate().add(1); // 1 - 0.0005 = 0.9995
  if (userPrice.div(buyPrice).lt(validDifference)){
  ...
  this._totalSupply = this._totalSupply.add(tokensToIssue);
  // We should add tokens to buyer
  this.balances.set(buyerAddress, tokensToIssue);
  
  // We should write down, how much fees we earned...
  let profitFees = purchaseAmount.mul(this._buyPriceFee);
  this._totalEarnings = this._totalEarnings.add(profitFees);
  profitFees;
