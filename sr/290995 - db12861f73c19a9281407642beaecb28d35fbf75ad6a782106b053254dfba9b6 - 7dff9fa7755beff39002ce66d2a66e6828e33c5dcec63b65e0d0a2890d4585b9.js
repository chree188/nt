...
var Nvshen = function () {
    ...
    LocalContractStorage.defineProperty(this, 'currentIndex');
...
Nvshen.prototype = {
    ...
    get: function () {
        if (this.indexArr.length > 0) {
            this.currentIndex = parseInt(Math.random() * this.indexArr.length)
            let from = Blockchain.transaction.from
            let time = +new Date();
            this.accessLog = this.accessLog + '\n' + this.indexArr[this.currentIndex] + ',' + from + ',' + time;
            return this.indexArr[this.currentIndex];
        } else {
            return {}
        }
    },
