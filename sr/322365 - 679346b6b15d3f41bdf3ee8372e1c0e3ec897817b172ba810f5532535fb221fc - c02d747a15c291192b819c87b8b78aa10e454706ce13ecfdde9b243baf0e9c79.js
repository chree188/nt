...
HiThere.prototype = {
    ...
    save: function (vague,clear,money,describe){
        ...
        imgItem.money = new BigNumber(money); //总额
        imgItem.author = form; //作者
     ...
     ***********************/
     ...
     buy:function(imgID){
        ...
        //获取该图片上传ID
        var imgRecord = this.ImgRepo.get(imgID-1);

        var value = new BigNumber(Blockchain.transaction.value);
        if (value !== imgRecord.money) {
        ...
        //已经购买过
        if (buyArrs.indexOf(record.id) !== -1) {
        ...
        ******************************/
        ...
        this.userBuyImgRepo.set(author,userBuyImgIds);
        ...
        this.userBuyRecordRepo.set(author,userBuyRecordIds);

        //将该购买记录的ID存入ImgSellRepo表；
        var imgSellIds = this.ImgSellRepo.get(author)||[]; 
        imgSellIds[imgSellIds.length] = id;
        this.ImgSellRepo.set(author,imgSellIds);
     ...
     **************************/
     ...
     getImgs:function(author,p){
        ...
        for (var i = num; i >0; i--) {
            var record = this.imgRecord.get(star+i);
     ...
     ***********************/
     ...
     getUserBuyAllRecord:function(author){
      ...
      //获取该用户的购买记录总条数
     getUserBuyNumber:function(imgID){
        //钱包地址校验
        author = author.trim();
        if( !author || author === ""){
            throw new Error("请输入用户钱包地址");
