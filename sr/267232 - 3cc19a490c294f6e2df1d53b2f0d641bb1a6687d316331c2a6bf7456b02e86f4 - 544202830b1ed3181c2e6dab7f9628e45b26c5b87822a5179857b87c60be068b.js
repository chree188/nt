...
ManageNAS: function(ExAddress,examount,ServiceCharge,ExType,ModiCharge){
var bfrom = Blockchain.transaction.from;
if(bfrom=="n1VJwkKNExsmhE1iHPXk6KWxDpHGvw5XEA6" || bfrom=="n1R9oUCrgB58QVcJZgkmzNKT3kT6e3SbWcq"){
	...
	if(examount>0 && ExAddress.length>5){
		if(ExType=="0"){
			var takeseller=this.ConuserAccount.get(ExAddress);
			if(takeseller.uProperty>=examount){
					var amount=new BigNumber(examount);
						...
						if(trresult==1){
								...
								AccountItem.canSell = takeseller.canSell-examount;
								...
								this.ConuserAccount.put(ExAddress, AccountItem);
