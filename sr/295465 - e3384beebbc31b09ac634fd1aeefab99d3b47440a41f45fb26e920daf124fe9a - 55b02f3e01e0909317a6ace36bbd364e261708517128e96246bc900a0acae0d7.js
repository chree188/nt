...
SmartContract.prototype = {
  ...
  init: function (terms, minimumAmount, maximumAmount, cancellationFee) { // OK
    ...
    // Setting all numbers to zero, otherwise we will need to make a check if it is not null in other methods.
    let zeroBigNumber = new BigNumber(0);
    this.supply = zeroBigNumber;
    this.paid = zeroBigNumber;
    this.deposits = zeroBigNumber;
    this.interests = zeroBigNumber;
    this.cancelled = zeroBigNumber;
    this.depositsAmount = zeroBigNumber;
    this.depositsCount = zeroBigNumber;
    this.withdrawalsAmount = zeroBigNumber;
    this.withdrawalsCount = zeroBigNumber;
    this.cancelledAmount = zeroBigNumber;
    this.cancelledCount = zeroBigNumber;
  ...
  // Check if transaction is empty (has value === 0). Otherwise there might be some accidential transfers we don't want.
  _isEmpty: function (){ // OK
    // We use transaction property, to determine if enything was transfered.
    let incomingPayment = Blockchain.transaction.value;
    if (incomingPayment.eq(0)){ // No need to transfer value here.
      return true;
    }
    else {
      return false;
  ...
  _notify: function (eventType, eventData){ // NOT TESTED
    let eventObject = {};
  ...
  edit: function (settingsObject){ // OK
    if (!this._isOwner()){
    ...
    for (var setting in settingsObject){
      ...
      if (setting === "terms"){
        ...
        this.cancellationFee = new BigNumber(newValue || 0);
      }
      else if (setting === "minimumAmount"){
        previousValue = this.minimumAmount;
        this.minimumAmount = new BigNumber(newValue || 1);
      }
      else if (setting === "maximumAmount"){
        previousValue = this.maximumAmount;
        this.maximumAmount = new BigNumber(newValue || 1);
      }
  ...
  withdraw: function () {
    ...
    if (depositAmount.eq(0)){ // No deposit here.
      ...
      // Time to withdraw funds...
        ...
        this.deposit.set(withdrawalAddress, new BigNumber(0));
        this.interest.set(withdrawalAddress, new BigNumber(0));
  ...
  cancel: function () {
    ...
    if (depositAmount.eq(0)){ // No deposit here.
      ...
      // Cancellation fee goes to our supply, so we can pay interest for other deposits. As well as non-paid interest for this deposit.
      this.supply = this.supply.add(cancellationFee.add(interestAmount));
  ...
  create: function (rawTermIndex) { // OK
    ...
    if (isNaN(tarifIndex) || this.terms[tarifIndex] === undefined){
      throw new Error("Wrong index of deposit term, please use method 'info' to check terms available.");
    }
    ...
    if (interestAmount.lt(1)){
      throw new Error("Interest amount is too small, please increase deposit or choose different term.");
    }
    else if (interestAmount.gt(currentSupply)){
      throw new Error("Supply " + currentSupply + " is too small to provide you with requested interest, please reduce deposit amount or choose different term.");
    }
    ...
    this.supply = currentSupply.sub(interestAmount);
  ...
  addSupply: function () { // OK
    ...
    if (depositAmount.lt(1)){
      throw new Error("Bad supply amount");
    }
    
    // Increase our supplies
    this.supply = this.supply.add(depositAmount);
    
  },
  
  // Retrieve unused supply
  retrieveSupply: function (retrieveAmountRAW) { // OK
    ...
    let retrieveAmount = new BigNumber(retrieveAmountRAW);
    let currentSupply = this.supply;
