...
var tokenConfig = {
  "NASDecimals": 18, // To allow us to use human-readable values during transfers, we need to constantly change from price in NAS to price in Wei.
...
var StandardToken = function () {
  ...
  LocalContractStorage.defineProperties(this, {
    ...
    "_decimals": {
      ...
      stringify: function (bigNumberObject) {
        ...
        else if (bigNumberObject.lt(tokenConfig.decimals.minimum) || bigNumberObject.gt(tokenConfig.decimals.maximum)){
          throw new Error("Decimals must be non-negative Integer between " + tokenConfig.decimals.minimum + ' and ' + tokenConfig.decimals.maximum);
        } 
        else {
          // We receive integer number, like 18 ot 9 for store. But we store 10 ^ 18 as string, because it is always used for multiplying supplies. It should be much more convenient...
          return bigNumberObject.toString(10);
        }
      }
    },
    // DecimalsValue, number like 10 ^ 9. No checks need here
    "_decimalsValue": {
      parse: function (storedIntegerString) {
        return new BigNumber(storedIntegerString);
      },
    ...
    "_totalSupply": {
      ...
      stringify: function (bigNumberObject) {
        // Check that value is Integer, as it must be.
        if (!bigNumberObject.isInteger()){
          throw new Error('totalSupply must be Integer');
        }
        // It must be non-negative
        else if (bigNumberObject.lt(0)){
          throw new Error('totalSupply must be non-negative');
        } 
        else {
          return bigNumberObject.toString(10);
        }
  ...
  LocalContractStorage.defineMapProperties(this, {
    ...
    "balances": {
      ...
      stringify: function (bigNumberObject) {
        if (!bigNumberObject.isInteger() || bigNumberObject.lt(0)){
          throw new Error('Balance must be non-negative Integer');
        }
        else {
          return bigNumberObject.toString(10);
        }
    ...
    "allowed": {
      ...
      stringify: function (bigNumberObject) {
        if (!bigNumberObject.isInteger() || bigNumberObject.lt(0)){
          throw new Error('Allowance must be non-negative Integer');
        }
        else {
          return bigNumberObject.toString(10);
        }
  ...
  LocalContractStorage.defineProperties(this, {
    ...
    "_priceValidBlocks": {
      ...
      stringify: function (bigNumberObject) {
        if (!bigNumberObject.isInteger() || bigNumberObject.lt(1)){
          throw new Error('priceValidBlocks must be positive Integer');
        }
        else {
          return bigNumberObject.toString(10);
        }
...
StandardToken.prototype = {
  ...
  init: function (name, symbol, decimals, totalSupply, description, help, tags, userInterface) {
    ...
    let decimalsValue = new BigNumber(10).pow(this._decimals);
    this._decimalsValue = decimalsValue;
...
StandardToken.prototype.recalculatePrices = function () {
  ...
  let contractBalanceNAS = contractBalance.div(new BigNumber(10).pow(tokenConfig['NASDecimals']));
  ...
  if (this.nasPrice && this.goldPrice){
    let buyPriceWithPrecision = buyPrice.precision(5);
    let sellPriceWithPrecision = sellPrice.precision(5);
...
StandardToken.prototype.purchase = function (userBuyPrice) {
  ...
  let receivedNAS = receivedWei.div(new BigNumber(10).pow(tokenConfig['NASDecimals'])); // BigNumber
  ...
  let tokensToIssue = purchaseAmount.mul(this._decimalsValue).decimalPlaces(0); // Only integer number of tokens can be used! No checks, just take care about it by ourselves.
  ...
  let profitFees = purchaseAmount.mul(this._buyPriceFee).mul(new BigNumber(10).pow(tokenConfig['NASDecimals']));
...
StandardToken.prototype.dispose = function (userSellPrice, userSellGold) {
  ...
  let NASMultiplicator = new BigNumber(10).pow(tokenConfig['NASDecimals']); // For convenience and to use less CPU cycles.
  
  let currentNASBalance = this._contractBalance || new BigNumber(0); // In WEI
  let requiredNAS = gramsToBurn.mul(userPrice);
  let requiredWei = requiredNAS.mul(NASMultiplicator).decimalPlaces(0); // must be integer...
  ...
  return {
    ...
    'disposedGold': tokensToBurn, // How many GOLD disposed
